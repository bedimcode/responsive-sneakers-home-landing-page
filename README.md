# Responsive Sneakers Landing Page 

This project is part of a step-by-step YouTube tutorial in which we build a responsive sneakers landing page from scratch using HTML, CSS and JavaScript.

## 📌 Project Features
- Fully responsive sneakers home landing page (Mobile First Methodology).
- Clear and semantic HTML structure.
- Use of CSS variables and modern JavaScript functions.
- Smooth animations with GSAP.
- Elegant glides with Swiper Js.
- Compatible with all modern browsers and devices.

## 🚀 Watch the full tutorial on YouTube  
🎬 [Watch the Demo & Code](https://youtu.be/L0JaHTxRAMQ)

### 📦 Download the resources directly
1. Click the green **code** button.
2. Click **Download ZIP**.
3. Extract the ZIP file and open the project in your code editor.

### 📥 Clone or Fork the repository
You can **clone** the repository to your local machine or **fork** it in your GitHub account to get all the project's resources.

```bash
# Clone the repository using HTTPS
git clone https://github.com/bedimcode/responsive-sneakers-home-landing-page.git
```

Designed & developed with ❤️ by **[Bedimcode](https://www.youtube.com/@Bedimcode)**

![preview img](/preview.png)
